...
***** unreal ~DoCast~ Ternary operators
	...
	bool UObjectBaseUtility::IsA( const UClass* SomeBase ) const
	{
      ...
      condition of a look. In unreal, it is a ~UClass~ pointer.

      How can we use a pointer to set condition within a ~for~ loop?

      Now, I just want to know:

      How Unreal do runtime check to see if `From` is a child of `To`?
      If I had somehow to write a piece code to do this kind of check,
      I would do the following snippet:
